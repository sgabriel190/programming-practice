 ...
 * players of some team standing one after another, then the situation is considered dangerous. For example, the
 * situation 00100110111111101 is dangerous and 11110111011101 is not. You are given the current situation.
...
fun main(){
    ...
    var subSequenceCounter = 1
    for(index in input.indices) {
        if (index != input.length - 1) {
            if (input[index] == input[index + 1]) subSequenceCounter++ else subSequenceCounter = 1
        }
        if (subSequenceCounter == 7) break
    }
    if(subSequenceCounter==7) println("YES") else println("NO")
}
