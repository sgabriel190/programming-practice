/**
 * Little Petya loves presents. His mum bought him two strings of the same size for his birthday. The strings consist
 * of uppercase and lowercase Latin letters. Now Petya wants to compare those two strings lexicographically. The letters'
 ...
 */
fun main(){
    val string1 = readLine()!!.trim()
    val string2 = readLine()!!.trim()

    if(string1.compareTo(string2, ignoreCase = true) == 0){
        println("0")
    }
    else if(string1.compareTo(string2, ignoreCase = true) > 0){
        println("1")
    }else{
        println("-1")
    }
